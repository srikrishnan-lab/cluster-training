# This file uses the parametric bootstrap in serial to sample parameters
...
# you could also do this with apply if you'd made a more clever wrapper function
params_boot <- matrix(NA, nrow=nboot, ncol=4) # Initialize storage
start.time <- Sys.time() # start timing
for (i in 1:nboot) {
    fit <- optim(start, rmse, gr=NULL, func=sim_sl, time=yrs, dat=sl_boot[i, ])
    params_boot[i, ] <- fit$par
}
